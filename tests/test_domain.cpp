...
TEST_CASE("Making domain", "[Domain]") {
  ...
  Logger::print_debug("particles[0] initially has {} particles", data.particles[0].number());
  ...
  data.particles[0].append({0.1f, 0.2f, 0.3f}, {0.0f, 0.0f, 0.0f},
                           mesh.get_idx(148, 32),
                           ParticleType::electron);
  Logger::print_debug("particles[0] now has {} particles", data.particles[0].number());
  Logger::print_debug("particles[1] initially has {} particles", data.particles[1].number());
  ...
  data.particles[1].append({0.32f, 0.15f, 0.26f}, {0.0f, 0.0f, 0.0f},
                           mesh.get_idx(229, 12),
                           ParticleType::positron);
  Logger::print_debug("particles[1] now has {} particles", data.particles[1].number());
  data.send_particles();

  Logger::print_debug("x1s: {}, {}, {}", data.particles[0].data().x1[0],
                      data.particles[0].data().x1[1], data.particles[0].data().x1[2]);
  Logger::print_debug("cells: {}, {}, {}", data.particles[0].data().cell[0],
                      data.particles[0].data().cell[1], data.particles[0].data().cell[2]);
