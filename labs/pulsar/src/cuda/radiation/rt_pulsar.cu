...
count_pairs_produced(PhotonData photons, size_t number, int *pair_count,
  ...
  for (uint32_t tid = id; tid < number; tid += blockDim.x * gridDim.x) {
    ...
    // Scalar B1v =
    //     interp(b1, photons.x1[tid], photons.x2[tid], c1, c2,
    //     Stagger(0b001));
    // Scalar B2v =
    //     interp(b2, photons.x1[tid], photons.x2[tid], c1, c2,
    //     Stagger(0b010));
    // Scalar B3v =
    //     interp(b3, photons.x1[tid], photons.x2[tid], c1, c2,
    //     Stagger(0b100));
...
RadiationTransferPulsar::RadiationTransferPulsar(
  ...
  d_rand_states.resize(env.dev_map().size());
  // m_numPerBlock.resize(env.dev_map().size());
  // m_cumNumPerBlock.resize(env.dev_map().size());
  // m_posInBlock.resize(env.dev_map().size());
  for_each_device(env.dev_map(), [this](int n) {
    ...
    init_rand_states((curandState *)d_rand_states[n], seed + n,
                     m_threadsPerBlock, m_blocksPerGrid);

    m_numPerBlock.emplace_back(m_blocksPerGrid, m_env.dev_map()[n]);
    m_cumNumPerBlock.emplace_back(m_blocksPerGrid, m_env.dev_map()[n]);
    m_posInBlock.emplace_back(m_env.params().max_ptc_number,
                              m_env.dev_map()[n]);
  });
}
...
RadiationTransferPulsar::~RadiationTransferPulsar() {
  for_each_device(m_env.dev_map(), [this](int n) {
    CudaSafeCall(cudaFree((curandState *)d_rand_states[n]));
  });
}
...
RadiationTransferPulsar::emit_photons(cu_sim_data &data) {
  for_each_device(data.dev_map, [this, &data](int n) {
    ...
    m_posInBlock[n].assign_dev(0, ptc.number());
    m_numPerBlock[n].assign_dev(0);
    m_cumNumPerBlock[n].assign_dev(0);

    Kernels::count_photon_produced<particle_data>
        <<<m_blocksPerGrid, m_threadsPerBlock>>>(
            ptc.data(), ptc.number(), m_numPerBlock[n].data_d(),
            m_posInBlock[n].data_d(), (curandState *)d_rand_states[n],
            data.photon_produced[n].ptr());
    CudaCheckError();

    thrust::device_ptr<int> ptrNumPerBlock(m_numPerBlock[n].data_d());
    thrust::device_ptr<int> ptrCumNum(m_cumNumPerBlock[n].data_d());
...
RadiationTransferPulsar::produce_pairs(cu_sim_data &data) {
  for_each_device(data.dev_map, [this, &data](int n) {
    auto &ptc = data.particles[n];
    auto &photons = data.photons[n];
    m_posInBlock[n].assign_dev(0, photons.number());
    m_numPerBlock[n].assign_dev(0);
    m_cumNumPerBlock[n].assign_dev(0);
