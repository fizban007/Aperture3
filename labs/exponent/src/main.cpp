...
int main(int argc, char *argv[]) {
  ...
  // std::vector<Scalar> Es{0.01, 0.03, 0.06, 0.1,  0.3,   0.6,   1.0,   3.0,
  //                        6.0,  10.0, 30.0, 60.0, 100.0, 300.0, 600.0, 1000.0};
  ...
  for (auto Eacc : Es) {
    ...
    sim.prepare_initial_condition(100, 5.0e6);

    for (uint32_t step = 0; step < env.params().max_steps; step++) {
      sim.add_new_particles(10, 5.0e6);
      Logger::print_info(
      ...
      if (std::abs(Eacc - 0.0) < 0.001 && step % env.params().data_interval == 0) {
