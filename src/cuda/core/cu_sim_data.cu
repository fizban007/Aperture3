...
cu_sim_data::initialize(const cu_sim_environment &env) {
  ...
  // CudaSafeCall(cudaSetDevice(dev_id));
  Logger::print_debug("on device {}", dev_id);
  auto &mesh = grid->mesh();
  ...
  for (int i = 0; i < num_species; i++) {
    ...
    gamma.emplace_back(*grid);
    gamma[i].initialize();
    gamma[i].sync_to_host();

    ptc_num.emplace_back(*grid);
    ptc_num[i].initialize();
    ptc_num[i].sync_to_host();
  }

  // TODO: Finish the buffers for ptc and photons
...
cu_sim_data::fill_multiplicity(Scalar weight, int multiplicity) {
  Kernels::fill_particles<<<dim3(16, 16), dim3(32, 32)>>>(
      particles.data(), weight, multiplicity);
  CudaCheckError();

  auto &mesh = grid->mesh();
  particles.set_num(mesh.dims[0] * mesh.dims[1] * 2 * multiplicity);
  CudaSafeCall(cudaDeviceSynchronize());
}

void
cu_sim_data::init_grid(const cu_sim_environment &env) {
  // Setup the grid on device
  if (env.params().coord_system == "Cartesian") {
    grid.reset(new Grid());
  } else if (env.params().coord_system == "LogSpherical") {
    grid.reset(new Grid_LogSph_dev());
  } else if (env.params().coord_system == "1DGR" && grid->dim() == 1) {
...
//     CudaSafeCall(cudaMallocManaged(&(ptc_send_left[i]),
//     sizeof(int))); *ptc_send_left[i] = 0;
//     CudaSafeCall(cudaMallocManaged(&(ptc_send_right[i]),
//     sizeof(int))); *ptc_send_right[i] = 0;
//     CudaSafeCall(cudaMallocManaged(&(ph_send_left[i]), sizeof(int)));
//     *ph_send_left[i] = 0;
//     CudaSafeCall(cudaMallocManaged(&(ph_send_right[i]),
//     sizeof(int))); *ph_send_right[i] = 0;
//   }
...
//         photons[n].data(), photons[n].number(), ph_buffer[2 *
//         n].data(), ph_buffer[2 * n + 1].data(), last_dim,
//         ph_send_left[n], ph_send_right[n]);
//     CudaCheckError();
//   }
...
//         });
//     photons[n - 1].set_num(photons[n - 1].number() +
//     *ph_send_left[n]);
//   }
//   for (int n = 0; n < dev_map.size() - 1; n++) {
//     CudaSafeCall(cudaSetDevice(dev_map[n + 1]));
...
//   //     *ph_recv_left[i]); Logger::print_info("Sent {} particles
//   from
...
cu_sim_data::compute_edotb() {
  ...
  Kernels::compute_EdotB<<<gridSize, blockSize>>>(
      ...
      Bbg.ptr(0), Bbg.ptr(1), Bbg.ptr(2), EdotB.ptr());
  CudaCheckError();
}
