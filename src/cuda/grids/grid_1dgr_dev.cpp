...
Grid_1dGR_dev::init(const SimParams& params) {
  ...
  m_dpsidth.resize(n_size);
  m_dpsidth_j.resize(n_size);
  m_j0.resize(n_size);
  m_agrr.resize(n_size);
  m_agrf.resize(n_size);
  m_rho0.resize(n_size);

  // TODO: initialize all the array content according to a pre-computed
  // hdf5
  ...
  for (uint32_t n = 0; n < n_size; n++) {
    ...
    m_dpsidth_j[n] =
        ...
        sqrt_gamma;
    m_agrr[n] = std::sqrt(Delta * Sigma / A) * g11;
    // Scalar denom = (m_alpha2[n] - m_D3[n]) * m_D2[n] +
    // square(m_D1[n]);
    if (n % 10 == 0) {
      Logger::print_info(
          "rho0 is {}, rho0/dpsidth is {}, sqrt_gamma is {}, j^2 is {}, alpha*rho is "
          "{}",
          m_rho0[n], m_rho0[n]/m_dpsidth[n], sqrt_gamma, sqrt(g11) * m_j0[n],
          m_rho0[n] * m_alpha[n]);
    }
  }
  m_D1.sync_to_device();
  ...
  m_dpsidth.sync_to_device();
  m_dpsidth_j.sync_to_device();
  m_j0.sync_to_device();
  m_rho0.sync_to_device();
  m_agrr.sync_to_device();
  m_agrf.sync_to_device();

  HF::File test_out("debug.h5", HF::File::ReadWrite | HF::File::Create | HF::File::Truncate);
  HF::DataSet out_dpsidth = test_out.createDataSet<Scalar>("dpsidth", HF::DataSpace{m_dpsidth_j.size()});
  out_dpsidth.write(m_dpsidth_j.data());
}
...
Grid_1dGR_dev::get_mesh_ptrs() const {
  ...
  ptrs.dpsidth = m_dpsidth.data_d();
  ptrs.dpsidth_j = m_dpsidth_j.data_d();
  ptrs.j0 = m_j0.data_d();
