...
HOST_DEVICE double total_f(Scalar s) {
  if (s < 4.0) {
    return 0.0;
  } else if (s < 4.6) {
    Scalar s4 = s - 4.0;
    ...
    return 0.582814 - 0.29842 * s + 0.04354 * s * s - 0.0012977 * s * s * s;
  } else if (s < 14.0) {
    ...
    return l2s * 28.0 / 9.0 - 218.0 / 27.0 +
           (-(4.0 / 3.0) * l2s * l2s * l2s + 3.863 * l2s * l2s - 11.0 * l2s +
            27.9) /
               s;
  }
}

HOST_DEVICE double approx_Em(Scalar s) {
  if (s <= 4.01) {
    return 0.0;
  } else if (s <= 100.0) {
    return std::exp(
               (std::log(1.337) + std::log(5.2495 / 1.337) * std::log(s / 4.01) /
                                      std::log(100.0 / 4.01))) /
           s;
    // } else if (s < 100.0) {
    //   return 0.71 / sqrt(s);
...
__device__ int find_tpp_n_gamma(Scalar gamma) {
  if (gamma < MIN_GAMMA)
    return 0;
  if (gamma > MAX_GAMMA)
    return dev_params.n_gamma - 1;
...
triplet_pairs::triplet_pairs(const SimParams &params)
    : m_rate(params.n_gamma), m_Em(params.n_gamma), m_gammas(params.n_gamma),
      m_threads(256), m_blocks(256), m_generator(), m_dist(0.0, 1.0) {
  m_dg = (log(MAX_GAMMA) - log(MIN_GAMMA)) / ((Scalar)params.n_gamma - 1.0);
  for (uint32_t n = 0; n < m_gammas.size(); n++) {
    m_gammas[n] = exp(log(MIN_GAMMA) + m_dg * (Scalar)n);
  }
  m_gammas.sync_to_device();

  CudaSafeCall(cudaMemcpyToSymbol(Kernels::dev_tpp_dg, &m_dg, sizeof(Scalar)));
  ...
  CudaSafeCall(cudaMemcpyToSymbol(Kernels::dev_tpp_rate, &dev_tpp_rate,
                                  sizeof(Scalar *)));
  CudaSafeCall(
      cudaMemcpyToSymbol(Kernels::dev_tpp_Em, &dev_tpp_Em, sizeof(Scalar *)));
  CudaSafeCall(
      cudaMemcpyToSymbol(Kernels::dev_tpp_gammas, &dev_g, sizeof(Scalar *)));

  CudaSafeCall(
      cudaMalloc(&m_states, m_threads * m_blocks * sizeof(curandState)));
  init_rand_states((curandState *)m_states, params.random_seed, m_threads,
                   m_blocks);
}
...
template <typename F>
void triplet_pairs::init(const F &n_e, Scalar emin, Scalar emax, double n0) {
  const int N_mu = 100;
  const int N_e = 800;

  // Compute the gammas and rates for triplet pair production
  Logger::print_info("Pre-calculating the triplet rate and pair mean energy");
  ...
  for (uint32_t n = 0; n < m_rate.size(); n++) {
    ...
    for (int i_mu = 0; i_mu < N_mu; i_mu++) {
      ...
      for (int i_e = 0; i_e < N_e; i_e++) {
        ...
        Emean += 0.5f * n_e(e) * total_f(s) * approx_Em(s) * (1.0 - b * mu) * e;
...
template void triplet_pairs::init<Spectra::power_law_hard>(
    const Spectra::power_law_hard &n_e, Scalar emin, Scalar emax, double n0);
template void triplet_pairs::init<Spectra::power_law_soft>(
    const Spectra::power_law_soft &n_e, Scalar emin, Scalar emax, double n0);
template void
triplet_pairs::init<Spectra::black_body>(const Spectra::black_body &n_e,
                                         Scalar emin, Scalar emax, double n0);
template void triplet_pairs::init<Spectra::mono_energetic>(
    const Spectra::mono_energetic &n_e, Scalar emin, Scalar emax, double n0);
template void triplet_pairs::init<Spectra::broken_power_law>(
    const Spectra::broken_power_law &n_e, Scalar emin, Scalar emax, double n0);
