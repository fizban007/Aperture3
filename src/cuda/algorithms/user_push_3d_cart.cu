#ifndef __USER_PUSH_3D_CART_H__
#define __USER_PUSH_3D_CART_H__
...
user_push_3d_cart(data_ptrs& data, size_t idx, Scalar dt,
                  curandState& state) {
  ...
  if (!dev_mesh.is_in_bulk(c1, c2, c3)) {
    ptc.cell[idx] = MAX_CELL;
    return;
  }

  // Load particle quantities
  Interpolator3D<Spline::spline_t<N>> interp;
  ...
  if (!check_bit(flag, ParticleFlag::ignore_EM)) {
    Scalar E1 =
        ...
        // interp(dev_bg_fields.E1, x1, x2, x3, c1, c2, c3,
        //        Stagger(0b110))) *
        q_over_m;
    Scalar E2 =
    ...
    Scalar B1 =
        (interp(data.B1, x1, x2, x3, c1, c2, c3, Stagger(0b001)) +
         interp(data.Bbg1, x1, x2, x3, c1, c2, c3, Stagger(0b001))) *
        q_over_m;
    Scalar B2 =
        (interp(data.B2, x1, x2, x3, c1, c2, c3, Stagger(0b010)) +
         interp(data.Bbg2, x1, x2, x3, c1, c2, c3, Stagger(0b010))) *
        q_over_m;
    Scalar B3 =
