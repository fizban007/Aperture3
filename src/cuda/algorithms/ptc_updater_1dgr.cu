...
update_ptc_1dgr(data_ptrs data, size_t num,
  ...
  for (size_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < num;
    ...
    if (step % dev_params.data_interval == 0 &&
      ...
      Scalar theta =
          mesh_ptrs.theta[c] * x1 + mesh_ptrs.theta[c - 1] * nx1;
...
update_photon_1dgr(photon_data photons, size_t num,
  ...
  for (size_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < num;
    ...
    if (step % dev_params.data_interval == 0 &&
      ...
      Scalar Delta = (r * r - 2.0f * r + a * a);
      Scalar g_00 =
          -(1.0f + 2.0f * r * (r * r + a * a) / Sigma / Delta);
...
__global__ void
test_current(pitchptr<Scalar> j, uint32_t cell) {
  printf("current at %d is %f\n", cell, j(cell, 0));
}
...
ptc_updater_1dgr::update_particles(sim_data& data, double dt,
  ...
  if (data.particles.number() > 0) {
    ...
    for (int i = 0; i < m_env.params().current_smoothing; i++) {
      ...
      // Kernels::test_current<<<1, 1>>>(get_pitchptr(data.J.data(0)), 5000);
      // CudaCheckError();

      for (int sp = 0; sp < m_env.params().num_species; sp++) {
        Kernels::filter_current1d<<<256, 256>>>(
            get_pitchptr(data.Rho[sp].data()), get_pitchptr(m_tmp_j),
            mesh_ptrs.K1, m_env.is_boundary(0), m_env.is_boundary(1));
        CudaCheckError();
...
void
ptc_updater_1dgr::prepare_initial_condition(sim_data& data,
                                            int multiplicity) {
...
void
ptc_updater_1dgr::prepare_initial_photons(sim_data& data,
                                          int multiplicity) {
  const Grid_1dGR* g = dynamic_cast<const Grid_1dGR*>(&data.env.grid());
  if (g != nullptr) {
