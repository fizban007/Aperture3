...
void
parse_config(const std::string& filename, SimParams& params) {
  SimParams defaults;
  ...
  try {
    ...
    params.max_tracked = config->get_as<uint64_t>("max_tracked")
    ...
    params.inject_ions = config->get_as<bool>("inject_ions")
                             .value_or(defaults.inject_ions);
    ...
    params.E_ph =
        config->get_as<double>("E_ph").value_or(defaults.E_ph);
    params.E_ph_min =
    ...
    params.a = config->get_as<double>("a").value_or(defaults.a);
    params.damping_length = config->get_as<uint64_t>("damping_length")
                                .value_or(defaults.damping_length);

    params.l_curv =
        config->get_as<double>("l_curv").value_or(defaults.l_curv);
    params.e_curv =
        config->get_as<double>("e_curv").value_or(defaults.e_curv);
    params.l_ph =
        config->get_as<double>("l_ph").value_or(defaults.l_ph);
    ...
    if (mesh_table) {
      ...
      if (tile_size) {
        for (int i = 0; i < 3; i++)
          params.tile_size[i] = (*tile_size)[i];
      }
    ...
    if (sim_table) {
      params.coord_system =
          sim_table->get_as<std::string>("coord_system")
      ...
      params.update_fields = sim_table->get_as<bool>("update_fields")
    ...
    Logger::err("Config file parse error: {}", e.what());
    exit(1);
  }
}
