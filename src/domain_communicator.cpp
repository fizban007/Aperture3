...
#define INSTANTIATE_FUNCTIONS(type)                                  \
  template void DomainCommunicator::get_guard_cells_leftright<type>( \
      int dir, MultiArray<type>& array, CommTags leftright,          \
      const Grid& grid);                                             \
  ...
  template void DomainCommunicator::get_guard_cells<type>(           \
      MultiArray<type> & array, const Grid& grid);                   \
  template void DomainCommunicator::put_guard_cells<type>(           \
      MultiArray<type> & array, const Grid& grid, int component)
...
template <typename T>
void
DomainCommunicator::get_guard_cells(MultiArray<T>& array,
...
template <typename T>
void
DomainCommunicator::put_guard_cells(MultiArray<T>& array,
                                    const Aperture::Grid& grid,
                                    int stagger) {
  for (unsigned int i = 0; i < grid.dim(); i++) {
