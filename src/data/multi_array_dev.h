#ifndef _MULTIARRAY_DEV_H_
#define _MULTIARRAY_DEV_H_
...
/// header file.
template <typename T>
class multi_array_dev {
 public:
  typedef T data_type;
  typedef T* ptr_type;
  typedef multi_array_dev<T> self_type;
  ...
  explicit multi_array_dev(int width, int height = 1, int depth = 1,
                      int deviceId = 0);

  /// Alternative main constructor, takes in an @ref Extent object and
  /// initializes an array of the corresponding extent.
  explicit multi_array_dev(const Extent& extent, int deviceId = 0);

  /// Standard copy constructor.
  multi_array_dev(const self_type& other);

  /// Standard move constructor.
  multi_array_dev(self_type&& other);
...
#endif  // ----- #ifndef _MULTIARRAY_DEV_H_  -----
