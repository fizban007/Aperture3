...
void
Grid::setup_line_cache_f::operator()(const Metric& g, Grid& grid) const {
  ...
  for (int k = 0; k < mesh.dims[2]; k++) {
    ...
    for (int j = 0; j < mesh.dims[1]; j++) {
      ...
      for (int i = 0; i < mesh.dims[0]; i++) {
        ...
        grid.m_line_cache[0][idx] = Int([&g, &mesh, &pos](double x){
            return g.alpha(x, pos[1], pos[2]) * g.g11(x, pos[1], pos[2]);
          }, pos[0], pos[0] + mesh.delta[0]);
        // index 1 is \int \alpha\gamma_{\theta\theta} d\theta
        grid.m_line_cache[1][idx] = Int([&g, &mesh, &pos](double x){
            return g.alpha(pos[0], x, pos[2]) * g.g22(pos[0], x, pos[2]);
          }, pos[1], pos[1] + mesh.delta[1]);
        // index 2 is \int \alpha\gamma_{r\phi} dr past the position of B^r, but
        // it will be multiplied eventually with B^\phi
        grid.m_line_cache[2][idx] = Int([&g, &mesh, &pos](double x){
            return g.alpha(x, pos[1], pos[2]) * g.g13(x, pos[1], pos[2]);
          }, pos[0], pos[0] + mesh.delta[0]);
        // index 3 is \int \beta^r\sqrt{\gamma} d\theta, past the position of
        // B^\theta, but this is integrating the edge of our own grid. Draw the
        // grid to see what is going on
        grid.m_line_cache[3][idx] = Int([&g, &mesh, &pos](double x){
            return g.b1(pos[0] + 0.5*mesh.delta[0], x, pos[2]) * sqrt(g.det(pos[0] + 0.5*mesh.delta[0], x, pos[2]));
        ...
        for (int n = 0; n < 3; n++) {
          ...
          if (n == 0) {
            ...
            if (std::abs(pos[1]) < 1.0e-7) {
              area = 2.0 * mesh.delta[2] * Int([&g, &mesh, &pos](double x){
                  return sqrt(g.det(pos[0] + 0.5*mesh.delta[0], x, pos[2]));
                }, pos[1], pos[1] + 0.5*mesh.delta[1]);
            } else {
              area = mesh.delta[2] * Int([&g, &mesh, &pos](double x){
                  return sqrt(g.det(pos[0] + 0.5*mesh.delta[0], x, pos[2]));
                }, pos[1] - 0.5*mesh.delta[1], pos[1] + 0.5*mesh.delta[1]);
            }
          } else if (n == 1) {
            ...
            auto Int = _1D::GQ::GaussLegendreQuadrature<double, 64>();
            area = mesh.delta[2] * Int([&g, &mesh, &pos](double x){
                return sqrt(g.det(x, pos[1] + 0.5*mesh.delta[1], pos[2]));
              }, pos[0] - 0.5*mesh.delta[0], pos[0] + 0.5*mesh.delta[0]);
          } else if (n == 2) {
            // phi direction
            auto Int = _2D::GQ::GaussLegendreQuadrature<double, 64>();
            area = Int([&g, &pos](double x, double y){
