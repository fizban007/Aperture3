#ifndef _MULTI_ARRAY_DEV_IMPL_H_
#define _MULTI_ARRAY_DEV_IMPL_H_
...
template <typename T>
multi_array_dev<T>::multi_array_dev()
...
template <typename T>
multi_array_dev<T>::multi_array_dev(int width, int height, int depth,
                          int deviceId)
    : _data_d(make_cudaPitchedPtr(nullptr, 0, 0, 0)),
      _data_h(nullptr),
...
template <typename T>
multi_array_dev<T>::multi_array_dev(const Extent& extent, int deviceId)
    : multi_array_dev(extent.width(), extent.height(), extent.depth(),
                 deviceId) {}

template <typename T>
multi_array_dev<T>::multi_array_dev(const self_type& other)
    : _data_d(make_cudaPitchedPtr(nullptr, 0, 0, 0)),
      _data_h(nullptr),
...
template <typename T>
multi_array_dev<T>::multi_array_dev(self_type&& other)
...
template <typename T>
void
multi_array_dev<T>::alloc_mem(const Extent& ext, int deviceId) {
...
template <typename T>
void
multi_array_dev<T>::copy_from(const self_type& other) {
...
template <typename T>
void
multi_array_dev<T>::assign(const data_type& value) {
  std::fill_n(_data_h, _size, value);
}

template <typename T>
void
multi_array_dev<T>::assign_dev(const data_type& value) {
  CudaSafeCall(cudaSetDevice(_devId));
  if (_dim == 3) {
...
template <typename T>
auto
multi_array_dev<T>::operator=(const self_type& other) -> self_type& {
...
template <typename T>
auto
multi_array_dev<T>::operator=(self_type&& other) -> self_type& {
  if (_extent != other._extent) {
    _extent = other._extent;
    _size = _extent.size();
    find_dim();
...
template <typename T>
void
multi_array_dev<T>::resize(int width, int height, int depth, int deviceId) {
  _extent.width() = width;
...
template <typename T>
void
multi_array_dev<T>::resize(Extent extent, int deviceId) {
  resize(extent.width(), extent.height(), extent.depth(), deviceId);
}

template <typename T>
void
multi_array_dev<T>::find_dim() {
...
template <typename T>
void
multi_array_dev<T>::sync_to_device(int devId) {
...
template <typename T>
void
multi_array_dev<T>::sync_to_device() {
  sync_to_device(_devId);
}

template <typename T>
void
multi_array_dev<T>::sync_to_host() {
  CudaSafeCall(cudaSetDevice(_devId));
  // CudaSafeCall(cudaMemPrefetchAsync(_data, _size * sizeof(T),
  // cudaCpuDeviceId));
  cudaMemcpy3DParms myParms = {0};
...
#endif  // _MULTI_ARRAY_DEV_IMPL_H_
