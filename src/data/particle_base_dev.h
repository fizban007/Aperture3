#ifndef _PARTICLE_BASE_DEV_H_
#define _PARTICLE_BASE_DEV_H_
...
class particle_base_dev : public particle_interface {
 protected:
 ...
 public:
  ...
  explicit particle_base_dev(std::size_t max_num);

  particle_base_dev(const particle_base_dev<ParticleClass>& other);
  particle_base_dev(particle_base_dev<ParticleClass>&& other);

  /// Virtual destructor because we need to derive from this class.
  virtual ~particle_base_dev();
  ...
  void copy_from(const particle_base_dev<ParticleClass>& other,
...
};  // ----- end of class particle_base_dev -----

}  // namespace Aperture

// #include "data/detail/particle_base_impl_dev.hpp"

#endif  // _PARTICLE_BASE_DEV_H_
