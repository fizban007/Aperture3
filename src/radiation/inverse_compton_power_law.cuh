...
HOST_DEVICE
Scalar
InverseComptonPL1D<RandFunc>::draw_photon_u1p(Scalar e1p, Scalar gamma) {
  Scalar u1p;
...
template <typename RandFunc>
HOST_DEVICE
InverseComptonPL1D<RandFunc>::InverseComptonPL1D(const SimParamsBase& params, RandFunc& rng) :
    m_alpha(params.spectral_alpha), m_es(params.e_s),
    m_emin(params.e_min), m_mfp(params.photon_path), m_rng(rng) {}
...
HOST_DEVICE
Scalar
InverseComptonPL1D<RandFunc>::draw_photon_energy(Scalar gamma, Scalar p) {
...
template <typename RandFunc>
HOST_DEVICE
InverseComptonPL1D<RandFunc> make_inverse_compton_PL1D(const SimParamsBase& params, RandFunc& rng) {
