...
InverseComptonDummy<RandFunc>::InverseComptonDummy(
    ...
    : m_cutoff(params.E_cutoff),
      m_Eph(params.E_ph),
      m_lph(params.photon_path),
      m_rng(rng) {}

template <typename RandFunc>
HOST_DEVICE InverseComptonDummy<RandFunc>::~InverseComptonDummy() {}
...
template <typename RandFunc>
HOST_DEVICE Scalar
InverseComptonDummy<RandFunc>::draw_photon_energy(Scalar gamma,
                                                  Scalar p) {
  return m_Eph;
}
...
template <typename RandFunc>
InverseComptonDummy<RandFunc>
make_inverse_compton_Dummy(const SimParamsBase& params, RandFunc& rng) {
