...
int main(int argc, char *argv[])
{
  ...
  // for (int i = mesh.guard[0]; i < mesh.dims[0] - mesh.guard[0]; i++) {
    ...
    //                          (env.gen_rand() < env.params().track_percent ? (int)ParticleFlag::tracked : 0));
  //   for (int n = 0; n < ppc; n++) {
  ...
  for (int i = mesh.guard[0]; i < mesh.dims[0] - mesh.guard[0]; i++) {
    ...
    // double rho = jb * 0.5 * (1.85 - 130.0 / (80.0 + 250.0 * i / (double)mesh.reduced_dim(0))) / env.params().q_e;
    double rho = rho_gj(jb, i / (double)mesh.reduced_dim(0)) / env.params().q_e;
    ...
    // for (int n = 0; n < 0.5*((jb * initial_M)/env.params().q_e-std::abs(rho)); n++) {
    // for (int n = 0; n < 0.5*((jb * initial_M)/env.params().q_e); n++) {
      // data.particles.append(env.gen_rand(), 0.0 * sgn(2.0 * i / mesh.reduced_dim(0) - 1.3), i,
