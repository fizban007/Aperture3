...
field_func<Metric>::operator()(const Vec3<double>& pos, Vec3<double>& v,
  ...
  if (component == "EdotB") {
    ...
    dydh = std::sqrt(v_J.x * v_J.x + v_J.y * v_J.y) / std::sqrt(v_B.x * v_B.x + v_B.y * v_B.y);
  } else {
...
void
field_integrator<Metric>::integrate_across_flux(const std::string& quantity, int num_samples, std::vector<Float_t>& values) {
  ...
  for (int i = 0; i < num_samples; i++) {
    ...
    Vec3<double> start (m_grid.mesh().lower[0] + 5 * m_grid.mesh().delta[0], theta, 0.0);
...
template <typename Float_t>
void
field_integrator<Metric>::integrate_line(const std::string& quantity, const Vec3<double>& start_pos, double step_size, std::vector<Vec3<double> >& points, std::vector<Float_t>& values) {
  RungeKutta<double, double> rk(false, 0.1, true);
  field_func<Metric> f(m_E, m_B, m_J, Metric{}, quantity);

  rk.integrate_along_field_simple(f, m_grid.mesh(), start_pos, step_size);
...
template <typename Metric>
double
field_integrator<Metric>::sample(const std::string& quantity, const Vec3<double>& pos) {
  return 0.0;
}
