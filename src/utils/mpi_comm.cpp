...
#define INSTANTIATE_SEND(type)                                      \
  template void MPICommBase::send<type>(                            \
      int dest_rank, int tag, const type* values, int n) const;     \
  template void MPICommBase::send<type>(int dest_rank, int tag,     \
                                        const type& value) const;   \
  template void MPICommBase::Isend<type>(int dest_rank, int tag,    \
                                         const type* values, int n, \
                                         MPI_Request& request) const

#define INSTANTIATE_RECV(type)                                      \
  template void MPICommBase::recv<type>(int source_rank, int tag,   \
                                        type* values, int n,        \
                                        MPI_Status& status) const;  \
  template void MPICommBase::recv<type>(int source_rank, int tag,   \
                                        type* values, int n) const; \
  template void MPICommBase::recv<type>(int source_rank, int tag,   \
                                        type& value) const;         \
  template void MPICommBase::Irecv<type>(int source_rank, int tag,  \
                                         type* values, int n,       \
                                         MPI_Request& request) const

#define INSTANTIATE_SEND_RECV(type)                           \
  template void MPICommBase::send_recv<type>(                 \
      int source_rank, const type* src_values, int dest_rank, \
      type* dest_values, int tag, int n) const;               \
  template void MPICommBase::send_recv<type>(                 \
      int source_rank, const type& src_value, int dest_rank,  \
      type& dest_value, int tag) const

#define INSTANTIATE_SCAN(type)                                       \
  template void MPICommCartesian::scan<type>(                        \
      const type* send_buf, type* result_buf, int num, int scan_dir, \
      bool exclusive) const

#define INSTANTIATE_GATHER(type)                            \
  template void MPICommBase::gather<type>(                  \
      const type* send_buf, int sendcount, type* recv_buf,  \
      int recvcount, int root) const;                       \
  template void MPICommBase::all_gather<type>(              \
      const type* send_buf, int sendcount, type* recv_buf,  \
      int recvcount) const;                                 \
  template void MPICommBase::gather<type>(                  \
      const type* send_buf, int sendcount, int root) const; \
  template void MPICommBase::gather_inplace<type>(          \
      type * recv_buf, int recvcount, int root) const;      \
  template void MPICommBase::gatherv<type>(                 \
      const type* send_buf, int sendcount, type* recv_buf,  \
      int* recvcounts, int* displs, int root) const;        \
  template void MPICommBase::gatherv<type>(                 \
      const type* send_buf, int sendcount, int root) const; \
  template void MPICommBase::gatherv_inplace<type>(         \
      type * recv_buf, int* recvcounts, int* displs, int root) const
